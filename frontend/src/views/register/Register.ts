            ...
            form.addEventListener("submit", async (event) => {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        document.body.innerHTML = "<h1 class='text-center text-2xl font-bold'>Registration Successful!</h1>";
                    } else {
                        const errorResponse = await response.json();
                        console.error("Registration failed:", errorResponse.error);
                        document.body.innerHTML = `<h1 class='text-center text-2xl font-bold text-red-600'>Registration Failed: ${errorResponse.error}</h1>`;
                    }
                } catch (error) {
